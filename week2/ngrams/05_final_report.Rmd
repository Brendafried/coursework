...
Total number of lines for year_counts `r year_counts %>% summarize(n())-1`
Total number of lines for total_counts `r total_counts %>% summarize(n())-1`
# Part B
...
join_years_and_totals <- left_join(year_counts, total_counts, by = 'year') %>%
...
filtered <- join_years_and_totals %>%
  filter(term == "1883" | term == "1910" | term == "1950") %>%
  mutate(term = as.character(term))

filtered %>%
  ggplot(aes(x=year, y=proportion_of_mentions, color = as.factor(rev(term)))) +
  geom_line() +
...
join_years_and_totals %>%
  filter(term == "1883" | term == "1910" | term == "1950") %>%
  mutate(term = as.character(term)) %>%
  ggplot() +
  geom_line(aes(x=year, y=volume, color = as.factor(rev(term)))) +
  coord_cartesian(xlim=c(1850, 2012)) +
...
```{r plot-totals}
join_years_and_totals %>% group_by(year) %>% 
  summarize(count_per_year = sum(total_volume)) %>%
  ggplot() +
  geom_line(aes(x=year, y=count_per_year)) +
  scale_y_continuous(label=comma)
```
...
```{r compute-peaks}
peak_mentions <- join_years_and_totals %>%
  filter(year >= 1850) %>%
  group_by(term)%>%
  filter(proportion_of_mentions == max(proportion_of_mentions)) %>%
  mutate(peak_year = year, half_life = .5 * proportion_of_mentions) %>%
  ungroup() %>% view
...
df1 <- join_years_and_totals %>%
  select(term, year, proportion_of_mentions)
df2 <- peak_mentions %>%
  select(term, peak_year, half_life)

half_lives <- left_join(df1, df2, by="term") %>% 
  filter(term >= 1875 & term <= 1975) %>%
  filter(year >= peak_year) %>%
  group_by(term) %>%
  filter(proportion_of_mentions < half_life) %>%
  filter(year == min(year)) %>%
  mutate(half_life_year = year - peak_year) %>% view
...
```{r plot-half-lifes}
half_lives %>% ggplot() + 
  geom_point(aes(x = term, y = half_life_year)) +
  geom_smooth(aes(x = term, y = half_life_year))
```
...
## Your written answer
1900 took the longest to forget, 1942 took the shortest to forget.

Write up your answer to Part G here. Include code that shows the years with the smallest and largest half-lifes.

```{r}
min_max_half_life <- half_lives %>%
  arrange(desc(half_life_year)) %>%
  select(term, half_life_year)

head(min_max_half_life, 1)
tail(min_max_half_life, 1)
```
